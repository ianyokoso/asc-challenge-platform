...
export function CertificationTrackingTable({
  ...
  const handleCellClick = (
    ...
    if (
      ...
      setSelectedCertification({ ...certification, participant, date });
      setIsModalOpen(true);
    }
  };
  ...
  return (
    <Fragment>
        ...
        <div className="mb-4 flex items-center justify-between">
          <div>
            ...
            <p className="text-body-sm text-gray-600">
              참여자 {data.participants.length}명 · 인증일 {data.dates.length}
              일
              {(data.trackType === 'short-form' ||
                data.trackType === 'shortform') && (
          ...
          <Badge className="bg-primary/10 text-primary">{data.trackType}</Badge>
        </div>
                ...
                <tr>
                  ...
                  >
                    ...
                    >
                      ...
                      {sortField === 'name' &&
                        (sortDirection === 'asc' ? (
                          <ChevronUp
                            className="h-3 w-3 lg:h-4 lg:w-4 flex-shrink-0"
                            aria-hidden="true"
                          />
                        ) : (
                          <ChevronDown
                  ...
                  {data.dates.map((date, idx) => {
                    ...
                    return (
                      ...
                      `}
                        aria-label={`${format(dateObj, 'M월 d일 EEEE', {
                          locale: ko,
                        })}`}
                      >
                        <div className="text-xs lg:text-sm font-semibold text-gray-700 truncate">
                  ...
                  >
                    ...
                    >
                      <div
                        ...
                        title={participant.discordUsername}
                      >
                        <Avatar className="h-6 w-6 lg:h-8 lg:w-8 flex-shrink-0">
                          {participant.discordAvatarUrl ? (
                            ...
                            <div
                              className="h-full w-full flex items-center justify-center bg-primary/10 text-primary font-semibold text-xs lg:text-sm"
                              aria-hidden="true"
                            >
                    ...
                    >
                      <div className="flex flex-col items-center gap-0.5 lg:gap-1">
                        ...
                        <span className="text-xs text-gray-600 whitespace-nowrap hidden lg:inline">
                          {participant.totalCertified}/
                          {participant.totalRequired}
                        </span>
                      </div>
                    </td>
                    ...
                    {data.dates.map((date) => {
                      ...
                      const display = getCertificationDisplay(
                        cert.status,
                        cert.url,
                      );
                      const isWeekBoundary = isWeekStart(date, data.trackType);
                      const isMonthBoundary = isMonthStart(date);
                      const dateObj = parse(date, 'yyyy-MM-dd', new Date());
                      const dateLabel = format(dateObj, 'M월 d일', {
                        locale: ko,
                      });
                      ...
                      return (
                        ...
                        `}
                          ...
                          onClick={() => handleCellClick(cert, participant, date)}
                        >
                          <div
                            className={`
        ...
        <div className="mt-4 space-y-2" role="note" aria-label="테이블 범례">
          <div className="flex flex-wrap gap-3 lg:gap-4 text-xs lg:text-sm text-gray-600">
            <div className="flex items-center gap-1.5 lg:gap-2">
              <CheckCircle2
                className="h-3 w-3 lg:h-4 lg:w-4 text-green-600 flex-shrink-0"
                aria-hidden="true"
              />
              <span>인증 완료</span>
            </div>
            <div className="flex items-center gap-1.5 lg:gap-2">
              <Clock
                className="h-3 w-3 lg:h-4 lg:w-4 text-yellow-600 flex-shrink-0"
                aria-hidden="true"
              />
              <span>인증 대기</span>
            </div>
            <div className="flex items-center gap-1.5 lg:gap-2">
              <XCircle
                className="h-3 w-3 lg:h-4 lg:w-4 text-red-600 flex-shrink-0"
                aria-hidden="true"
              />
              <span>미인증</span>
            </div>
            <div className="flex items-center gap-1.5 lg:gap-2">
              <Minus
                className="h-3 w-3 lg:h-4 lg:w-4 text-gray-400 flex-shrink-0"
                aria-hidden="true"
              />
              <span>인증 불필요</span>
            </div>
          ...
          <div className="flex flex-wrap gap-3 lg:gap-4 text-xs lg:text-sm text-gray-500">
            <div className="flex items-center gap-1.5 lg:gap-2">
              <div
                className="w-2 h-2 lg:w-3 lg:h-3 border-l-2 border-l-blue-400 flex-shrink-0"
                aria-hidden="true"
              />
              <span className="truncate">
                주 시작 (숏폼=월, 롱폼/빌더=일, 세일즈=화)
              </span>
            </div>
            <div className="flex items-center gap-1.5 lg:gap-2">
              <div
                className="w-2 h-2 lg:w-3 lg:h-3 bg-blue-50 border border-blue-200 flex-shrink-0"
                aria-hidden="true"
              />
              <span>월 시작</span>
            </div>
            {(data.trackType === 'short-form' ||
              ...
              <div className="flex items-center gap-1.5 lg:gap-2">
                <div
                  className="w-2 h-2 lg:w-3 lg:h-3 bg-blue-100 border border-blue-300 flex-shrink-0"
                  aria-hidden="true"
                />
      ...
      {selectedCertification && selectedCertification.participant && (
